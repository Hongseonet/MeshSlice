...
public class SliceController : MonoBehaviour {
    ...
    {
		Debug.Log("en " + collision.gameObject.name);
    }

    private void OnCollisionExit(Collision collision)
    {
        Debug.Log("ex " + collision.gameObject.name);

        MeshSlicer.CustomPlane plane = new MeshSlicer.CustomPlane(Vector3.zero, Vector3.one, Vector3.one);
        Slice(plane);
    }
}
